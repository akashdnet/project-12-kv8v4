...
import { deleteGuestlist } from "@/utils/actions";
...
export default function DeleteConfirmModal({ eventId, onClose }: DeleteConfirmModalProps) {
  ...
  const handleDelete = async () => {
    try {
      await deleteGuestlist(eventId);
      setTimeout(() => toast.success("Post deleted successfully!"), 300); // Add a slight delay
      queryClient.invalidateQueries({ queryKey: ["guestlist"] });
      onClose();
    } catch (error) {
      setTimeout(() => toast.error("Failed to delete post!"), 300);
    }
  };
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center bg-black/50">
      <div className="w-1/3 p-6 bg-white/10 backdrop-blur-md rounded-xl shadow-lg border border-white/20 text-white text-center">
        ...
        <p className="mb-6">Do you really want to delete this event? This action cannot be undone.</p>
