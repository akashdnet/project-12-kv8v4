...
export default function VenueClient() {
  ...
  const [editingPost, setEditingPost] = useState<IGallery | null>(null);
  const [duplicatingPost, setDuplicatingPost] = useState<IGallery | null>(null);
  const [deletingPostId, setDeletingPostId] = useState<string | undefined>(undefined);
  const [searchTerm, setSearchTerm] = useState("");

  // Filter posts based on search term
  const filteredPosts = data?.filter((event: IGallery) =>
        ...
        <table className="w-full border-collapse text-white">
          <thead>
            <tr className="border-b border-white/30">
              <th className="p-3 text-left">Feature Image</th>
          ...
          <tbody>
            {filteredPosts?.map((event: IGallery) => (
              <tr
                key={event.slug}
              ...
              >
                ...
                <td className="p-2">{event.createdAt
